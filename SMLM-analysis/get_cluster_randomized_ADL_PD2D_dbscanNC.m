function randcluster = get_cluster_randomized_ADL_PD2D_dbscanNC(xyz,factor,flag)
...
mv = (max(x)-min(x)) * (max(y)-min(y));   % mean rectangular volume
